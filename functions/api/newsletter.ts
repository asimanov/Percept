...
export interface Env { BUTTONDOWN_API_KEY?: string; }
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    const body = await parseBody(request); const email = (body.email || "").toString().trim();
    if (!isValidEmail(email)) return new Response(JSON.stringify({ ok: false, error: "Valid email required" }), { status: 400 });
    if (!env.BUTTONDOWN_API_KEY) return new Response(JSON.stringify({ ok: true, dryRun: true }), { status: 200 });
    const res = await fetch("https://api.buttondown.email/v1/subscribers", {
      method: "POST", headers: { Authorization: `Token ${env.BUTTONDOWN_API_KEY}`, "content-type":"application/json" }, body: JSON.stringify({ email })
    });
    if (!res.ok) return new Response(JSON.stringify({ ok: false, error: `Buttondown error: ${res.status} ${await res.text()}` }), { status: res.status });
